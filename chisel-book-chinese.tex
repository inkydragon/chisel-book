...
一些高层次的原理图可以给你一个系统的整体概述，但是它们不是用来描述系统。
...
别理解错我的意思：VHDL和Verilog可以完美描述用于综合成ASIC的硬件模块。
...
然而，这就是Chisel吗？这个硬件被生成用于出书字符串？并不是这样，这里实际上是解释了Scala代码，并不代表着硬件设计的Hollo World程序。
...
你可以在github上记录一个issue来提供反馈或者评论或者发送一个plain、邮件。
