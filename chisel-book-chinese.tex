...
%% XXX: use zh-cn config.
\input{shared/chisel-cn.tex}
...
%% XXX: We don't need two title page for PDF.
% \begin{flushleft}
% \pagestyle{empty}
% \ \\
% \vspace{1cm}
...
%% XXX: print build time
\vspace{6cm} \emph{Version: \today}
\newpage
\end{flushleft}
...
\ref{fig:chisel:hello}展现的是一个通过Chisel描述的闪烁LED。你是否理解这个代码案例的细节是不重要的。我们将会介绍这些内容在以下的章节中。
...
%   \myref{https://github.com/freechipsproject/chisel3/wiki}{Chisel Wiki}包含了小的chisel的用户手册和更多信息的链接。
...
a Chisel course focusing on hardware generators, as a \myref{https://jupyter.org/}{Jupyter} notebook
    \myref{https://github.com/ucb-bar/generator-bootcamp}{Generator Bootcamp} 是chisel课程，专注在硬件生成器，
作为jupyter notebook。
...
project containing small exercises with testers and solutions. However, it is a bit outdated.
    \myref{https://github.com/ucb-bar/chisel-tutorial}{Chisel Tutorial} 提供了一个准备安装的项目，包括小的测试和解决答案的练习。
\item A \myref{https://github.com/ccelio/chisel-style-guide}{Chisel Style Guide} by Christopher Celio.
    \myref{https://github.com/ccelio/chisel-style-guide}{Chisel Style Guide}，作者是Christopher Celio。
...
另一个常见的方式，来自 Java 和 Scala，是去使用 \myref{https://en.wikipedia.org/wiki/Camel_case}{camelCase}，由几个单词组成的标识。
...
这个tick是\emph{不}用作推演的时钟的，而是作为一个作为电路的使能信号，逻辑上在$f_{tick}$下操作。
...
比较器可以从0开始反向生成一个字符，然后一个大的与门。
当我们向下数零的时候，比较器是一个大的或非门，这个从AISC的角度是更加便宜的。
...
一个交通灯控制器需要保证在从红灯转绿的切换期间，有一个状态是两条路都亮着禁止通行的信号灯（红色或是橘色）。
